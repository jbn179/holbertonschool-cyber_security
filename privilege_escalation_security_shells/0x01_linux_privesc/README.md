...
`choom` est référencé sur [GTFOBins](https://gtfobins.github.io/gtfobins/choom/) comme binaire permettant l'exécution arbitraire de commandes. Tout binaire accordé en `sudo` sans restriction (`(ALL) NOPASSWD`) et listé sur GTFOBins peut être utilisé pour lire des fichiers root ou obtenir un shell.
...
cat /tmp/flag
# → your flag is ef2cb0723e45a3b45d3d341c2afe2833
```
...
Le `strcpy` écrit depuis `rbp-0x70`.
Le `strcmp` lit depuis `rbp-0x21`.
...
```bash
cat /root/flag.txt
# → your flag is a646c6b968e6fd6b8985fdd0dc361a77
```
...
- `sudo -l` est toujours la première commande : même un seul binaire autorisé peut suffire.
- Les cron jobs root avec `*` dans un dossier writable sont vulnérables au tar wildcard injection.
- `strings` + `objdump` permettent de reverse engineer un binaire sans accès au code source.
- L'absence de stack canary permet d'écraser des zones mémoire adjacentes via `strcpy` sans bounds checking.
